...
output "ssh_example_command" {
  value = var.use_existing_key_name ? format("ssh -i ~/.ssh/%s.pem ec2-user@%s", var.existing_key_name, aws_instance.dev_box.public_dns) : format("ssh -i %s ec2-user@%s", local_file.private_key_pem[0].filename, aws_instance.dev_box.public_dns)
}
